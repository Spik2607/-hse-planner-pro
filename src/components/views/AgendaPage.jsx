...
export default function AgendaPage() {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader><CardTitle>📅 Agenda HSE</CardTitle></CardHeader>
        <CardContent className="flex flex-col items-center space-y-4">
          ...
          <div className="w-full space-y-2">
            ...
            <Button onClick={handleAddEvent} className="w-full">Ajouter événement</Button>
          </div>
        </CardContent>
      </Card>

      {/* Liste événements mois */}
      <Card>
        <CardHeader><CardTitle>📋 Événements du mois</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {events.length === 0 ? (
            <p className="text-center text-gray-500">Aucun événement enregistré.</p>
          ) : (
